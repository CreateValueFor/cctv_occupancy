...
def preprocess(img):
    ...
    localMax = cv2.morphologyEx(
        img, cv2.MORPH_CLOSE, maxKernel, None, None, 1, cv2.BORDER_REFLECT101)
    ...
    binaryImage = cv2.morphologyEx(
        binaryImage, cv2.MORPH_CLOSE, morphKernel, None, None, opIterations, cv2.BORDER_REFLECT101)

    # Flood fill (white + black):
    cv2.floodFill(binaryImage, mask=None, seedPoint=(
